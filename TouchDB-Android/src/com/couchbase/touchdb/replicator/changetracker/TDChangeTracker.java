...
public class TDChangeTracker implements Runnable {
		...
		if (filterName != null) {
			...
			if (filterParams != null) {
				for (String filterParamKey : filterParams.keySet()) {
					path += "&"
							+ URLEncoder.encode(filterParamKey)
							+ "="
							+ URLEncoder.encode(filterParams
	...
	public boolean receivedChunk(String line) {
		if (line.length() > 1) {
			try {
				Map<String, Object> change = (Map) TDServer.getObjectMapper()
				...
				Log.w(TDDatabase.TAG,
						"Exception parsing JSON in change tracker", e);
				return false;
			}
		}
		return true;
	...
	public boolean receivedChange(final Map<String, Object> change) {
		...
		// pass the change to the client on the thread that created this change
		// tracker
		if (client != null) {
	...
	public boolean start() {
		this.error = null;
		thread = new Thread(this, "ChangeTracker-"
				+ databaseURL.toExternalForm());
		thread.start();
		return true;
	}
