...
public class TDRouter implements Observer {
	...
	public boolean cacheWithEtag(String etag) {
		...
		String requestIfNoneMatch = connection
	...
	public Map<String, Object> getBodyAsDictionary() {
		try {
			...
			Map<String, Object> bodyMap = TDServer.getObjectMapper().readValue(
					contentStream, Map.class);
			return bodyMap;
		} catch (IOException e) {
			return null;
		}
	...
	public EnumSet<TDContentOptions> getContentOptions() {
		EnumSet<TDContentOptions> result = EnumSet
				.noneOf(TDContentOptions.class);
	...
	public boolean getQueryOptions(TDQueryOptions options) {
		...
		options.setGroupLevel(getIntQuery("group_level",
				options.getGroupLevel()));
	...
	public void start() {
		...
		// We're going to map the request into a method call using reflection
				...
				if (db == null) {
					...
					try {
						...
						Log.e(TDDatabase.TAG,
								"Error closing empty output stream");
					}
					sendResponse();
					return;
				}
		...
		if (docID != null && pathLen > 2) {
			...
			if (attachmentName.startsWith("_") && docID.startsWith("_design")) {
				...
				message = message
						.replaceFirst("_Attachment", "_DesignDocument");
		...
		// Log.d(TAG, "path: " + path + " message: " + message + " docID: " +
		// docID + " attachmentName: " + attachmentName);

		// Send myself a message based on the components:
		TDStatus status = new TDStatus(TDStatus.INTERNAL_SERVER_ERROR);
		try {
			Method m = this.getClass().getMethod(message, TDDatabase.class,
					String.class, String.class);
			status = (TDStatus) m.invoke(this, db, docID, attachmentName);
		} catch (NoSuchMethodException msme) {
			try {
				Method m = this.getClass().getMethod("do_UNKNOWN",
						TDDatabase.class, String.class, String.class);
				status = (TDStatus) m.invoke(this, db, docID, attachmentName);
			} catch (Exception e) {
		...
		if (status.getCode() != 0) {
			...
			if (connection.getResponseBody() != null) {
				ByteArrayInputStream bais = new ByteArrayInputStream(connection
						.getResponseBody().getJson());
	...
	public TDStatus do_UNKNOWN(TDDatabase db, String docID,
	...
	public TDStatus do_GETRoot(TDDatabase _db, String _docID,
	...
	public TDStatus do_GET_all_dbs(TDDatabase _db, String _docID,
			String _attachmentName) {
	...
	public TDStatus do_GET_session(TDDatabase _db, String _docID,
			String _attachmentName) {
	...
	public TDStatus do_POST_replicate(TDDatabase _db, String _docID,
			String _attachmentName) {
		...
		boolean createTarget = (createTargetBoolean != null && createTargetBoolean
				.booleanValue());
		Boolean continuousBoolean = (Boolean) body.get("continuous");
		boolean continuous = (continuousBoolean != null && continuousBoolean
				.booleanValue());
		...
		if (!cancel) {
			...
			if (filterName != null) {
				...
				Map<String, Object> filterParams = (Map<String, Object>) body
						.get("query_params");
	...
	public TDStatus do_GET_uuids(TDDatabase _db, String _docID,
			String _attachmentName) {
	...
	public TDStatus do_GET_active_tasks(TDDatabase _db, String _docID,
			String _attachmentName) {
		...
		for (TDDatabase db : server.allOpenDatabases()) {
			...
			if (activeReplicators != null) {
				for (TDReplicator replicator : activeReplicators) {
					...
					String status = String.format("Processed %d / %d changes",
							processed, total);
					int progress = (total > 0) ? Math.round(100 * processed
							/ (float) total) : 0;
	...
	public TDStatus do_GET_Database(TDDatabase _db, String _docID,
			String _attachmentName) {
	...
	public TDStatus do_PUT_Database(TDDatabase _db, String _docID,
			String _attachmentName) {
		if (db.exists()) {
	...
	public TDStatus do_DELETE_Database(TDDatabase _db, String _docID,
			String _attachmentName) {
		if (getQuery("rev") != null) {
			return new TDStatus(TDStatus.BAD_REQUEST); // CouchDB checks for
														// this; probably meant
														// to be a document
														// deletion
		}
		return server.deleteDatabaseNamed(db.getName()) ? new TDStatus(
				TDStatus.OK) : new TDStatus(TDStatus.NOT_FOUND);
	}

	public TDStatus do_POST_Database(TDDatabase _db, String _docID,
			String _attachmentName) {
		TDStatus status = openDB();
		if (!status.isSuccessful()) {
			return status;
	...
	public TDStatus do_GET_Document_all_docs(TDDatabase _db, String _docID,
			String _attachmentName) {
	...
	public TDStatus do_POST_Document_bulk_docs(TDDatabase _db, String _docID,
			String _attachmentName) {
		Map<String, Object> bodyDict = getBodyAsDictionary();
		if (bodyDict == null) {
			return new TDStatus(TDStatus.BAD_REQUEST);
		}
		List<Map<String, Object>> docs = (List<Map<String, Object>>) bodyDict
				.get("docs");

		boolean allObj = false;
		if (getQuery("all_or_nothing") == null
				|| (getQuery("all_or_nothing") != null && (new Boolean(
						getQuery("all_or_nothing"))))) {
			allObj = true;
		}
		// allowConflict If false, an error status 409 will be returned if the
		// insertion would create a conflict, i.e. if the previous revision
		// already has a child.
		boolean allOrNothing = (allObj && allObj != false);
		boolean noNewEdits = true;
		if (getQuery("new_edits") == null
				|| (getQuery("new_edits") != null && (new Boolean(
						getQuery("new_edits"))))) {
			noNewEdits = false;
		}
		boolean ok = false;
		...
		try {
			for (Map<String, Object> doc : docs) {
				...
				if (noNewEdits) {
					...
					if (rev.getRevId() == null || rev.getDocId() == null
						...
						List<String> history = TDDatabase
								.parseCouchDBRevisionHistory(doc);
						status = db.forceInsert(rev, history, null);
					}
				} else {
					TDStatus outStatus = new TDStatus();
					rev = update(db, docID, docBody, false, allOrNothing,
							outStatus);
				...
				if (status.isSuccessful()) {
					...
					return status; // all_or_nothing backs out if there's any
									// error
					...
					return status; // abort the whole thing if something goes
									// badly wrong
				}
				if (result != null) {
					results.add(result);
			...
			Log.w(TDDatabase.TAG, String.format(
					"%s finished inserting %d revisions in bulk", this,
					docs.size()));
			ok = true;
		} catch (Exception e) {
			Log.w(TDDatabase.TAG, String.format(
					"%s: Exception inserting revisions in bulk", this), e);
		} finally {
	...
	public TDStatus do_POST_Document_revs_diff(TDDatabase _db, String _docID,
			String _attachmentName) {
		...
		if (db.findMissingRevisions(revs) != null) {
	...
	public TDStatus do_POST_Document_compact(TDDatabase _db, String _docID,
			String _attachmentName) {
		TDStatus status = _db.compact();
		if (status.getCode() < 300) {
			TDStatus outStatus = new TDStatus();
			outStatus.setCode(202); // CouchDB returns 202 'cause it's an async
									// operation
			return outStatus;
		} else {
			return status;
	...
	public TDStatus do_POST_Document_ensure_full_commit(TDDatabase _db,
			String _docID, String _attachmentName) {
		return new TDStatus(TDStatus.OK);
	}
	...
	public Map<String, Object> responseBodyForChanges(List<TDRevision> changes,
			long since) {
		List<Map<String, Object>> results = new ArrayList<Map<String, Object>>();
	...
	public Map<String, Object> responseBodyForChangesWithConflicts(
			List<TDRevision> changes, long since) {
		// Assumes the changes are grouped by docID so that conflicts will be
		// adjacent.
		...
		for (TDRevision rev : changes) {
			...
			if (docID.equals(lastDocID)) {
				Map<String, Object> changesDict = new HashMap<String, Object>();
				changesDict.put("rev", rev.getRevId());
				List<Map<String, Object>> inchanges = (List<Map<String, Object>>) lastEntry
						.get("changes");
		...
		Collections.sort(entries, new Comparator<Map<String, Object>>() {
			public int compare(Map<String, Object> e1, Map<String, Object> e2) {
				return TDMisc.TDSequenceCompare((Long) e1.get("seq"),
						(Long) e2.get("seq"));
			}
		});
	...
	public void sendContinuousChange(TDRevision rev) {
		Map<String, Object> changeDict = changesDictForRevision(rev);
		try {
			String jsonString = TDServer.getObjectMapper().writeValueAsString(
					changeDict);
			if (callbackBlock != null) {
				...
				try {
					...
					Log.e(TDDatabase.TAG,
							"IOException writing to internal streams", e);
				}
			}
		} catch (Exception e) {
		...
		if (observable == db) {
			...
			if (longpoll) {
				...
				if (callbackBlock != null) {
					byte[] data = null;
					try {
						data = TDServer.getObjectMapper().writeValueAsBytes(
								body);
					} catch (Exception e) {
				...
				Log.w(TDDatabase.TAG,
						"TDRouter: Sending continous change chunk");
	...
	public TDStatus do_GET_Document_changes(TDDatabase _db, String docID,
			String _attachmentName) {
		...
		if (continuous || (longpoll && changes.size() == 0)) {
			...
			if (options.isIncludeConflicts()) {
				connection.setResponseBody(new TDBody(
						responseBodyForChangesWithConflicts(changes, since)));
			} else {
				connection.setResponseBody(new TDBody(responseBodyForChanges(
						changes, since)));
			}
			return new TDStatus(TDStatus.OK);
		}
	...
	public String getRevIDFromIfMatchHeader() {
		...
		if (ifMatch.length() > 2 && ifMatch.startsWith("\"")
				&& ifMatch.endsWith("\"")) {
	...
	public TDStatus do_GET_Document(TDDatabase _db, String docID,
			String _attachmentName) {
		...
		if (openRevsParam == null || isLocalDoc) {
			...
			if (isLocalDoc) {
				...
				// ?atts_since parameter - value is a (URL-encoded) JSON array
				// of one or more revision IDs.
				// The response will include the content of only those
				// attachments that changed since the given revision(s).
				// (You can ask for this either in the default JSON or as
				// multipart/related, as previously described.)
				List<String> attsSince = (List<String>) getJSONQuery("atts_since");
				if (attsSince != null) {
					String ancestorId = db.findCommonAncestorOf(rev, attsSince);
					if (ancestorId != null) {
						int generation = TDRevision
								.generationFromRevID(ancestorId);
			...
			if (cacheWithEtag(rev.getRevId())) {
				return new TDStatus(TDStatus.NOT_MODIFIED); // set ETag and
															// check conditional
															// GET
			}
			...
			if (openRevsParam.equals("all")) {
				...
				TDRevisionList allRevs = db.getAllRevisionsOfDocumentID(docID,
						true);
				...
				// ?open_revs=[...] returns an array of revisions of the
				// document:
				...
				for (String revID : openRevs) {
					TDRevision rev = db.getDocumentWithIDAndRev(docID, revID,
							options);
					if (rev != null) {
						Map<String, Object> dict = new HashMap<String, Object>();
						dict.put("ok", rev.getProperties());
	...
	public TDStatus do_GET_Attachment(TDDatabase _db, String docID,
			String _attachmentName) {
		// http://wiki.apache.org/couchdb/HTTP_Document_API#GET
		EnumSet<TDContentOptions> options = getContentOptions();
		...
		if (cacheWithEtag(rev.getRevId())) {
			return new TDStatus(TDStatus.NOT_MODIFIED); // set ETag and check
														// conditional GET
		}

		String type = null;
		TDStatus status = new TDStatus();
		String acceptEncoding = connection
				.getRequestProperty("Accept-Encoding");
		TDAttachment contents = db.getAttachmentForSequence(rev.getSequence(),
				_attachmentName, status);
	...
	/**
	 * NOTE this departs from the iOS version, returning revision, passing
	 * status back by reference
	 */
	public TDRevision update(TDDatabase _db, String docID, TDBody body,
			boolean deleting, boolean allowConflict, TDStatus outStatus) {
		boolean isLocalDoc = docID != null && docID.startsWith(("_local"));
		String prevRevID = null;

		if (!deleting) {
			Boolean deletingBoolean = (Boolean) body
					.getPropertyForKey("deleted");
	...
	public TDStatus update(TDDatabase _db, String docID,
			Map<String, Object> bodyDict, boolean deleting) {
	...
	public TDStatus do_PUT_Document(TDDatabase _db, String docID,
			String _attachmentName) {
		Map<String, Object> bodyDict = getBodyAsDictionary();
		if (bodyDict == null) {
			return new TDStatus(TDStatus.BAD_REQUEST);
		...
		if (getQuery("new_edits") == null
			...
			// PUT with new_edits=false -- forcible insertion of existing
			// revision:
			TDBody body = new TDBody(bodyDict);
			...
			List<String> history = TDDatabase.parseCouchDBRevisionHistory(body
					.getProperties());
			return db.forceInsert(rev, history, null);
		}
	}

	public TDStatus do_DELETE_Document(TDDatabase _db, String docID,
			String _attachmentName) {
		return update(_db, docID, null, true);
	}

	public TDStatus updateAttachment(String attachment, String docID,
		...
		TDRevision rev = db.updateAttachment(attachment, contentStream,
				connection.getRequestProperty("content-type"), docID, revID,
				status);
		if (status.isSuccessful()) {
	...
	public TDStatus do_PUT_Attachment(TDDatabase _db, String docID,
			String _attachmentName) {
		return updateAttachment(_attachmentName, docID,
				connection.getRequestInputStream());
	}

	public TDStatus do_DELETE_Attachment(TDDatabase _db, String docID,
			String _attachmentName) {
	...
	public TDView compileView(String viewName, Map<String, Object> viewProps) {
		...
		TDViewMapBlock mapBlock = TDView.getCompiler().compileMapFunction(
				mapSource, language);
		if (mapBlock == null) {
			Log.w(TDDatabase.TAG, String
					.format("View %s has unknown map function: %s", viewName,
							mapSource));
			return null;
		}
		...
		if (reduceSource != null) {
			reduceBlock = TDView.getCompiler().compileReduceFunction(
					reduceSource, language);
			if (reduceBlock == null) {
				Log.w(TDDatabase.TAG, String.format(
						"View %s has unknown reduce function: %s", viewName,
						reduceBlock));
				return null;
			}
		}
	...
	public TDStatus queryDesignDoc(String designDoc, String viewName,
			List<Object> keys) {
		...
		if (view == null || view.getMapBlock() == null) {
			// No TouchDB view is defined, or it hasn't had a map block
			// assigned;
			// see if there's a CouchDB view definition we can compile:
			TDRevision rev = db.getDocumentWithIDAndRev(
					String.format("_design/%s", designDoc), null,
					EnumSet.noneOf(TDContentOptions.class));
			if (rev == null) {
				return new TDStatus(TDStatus.NOT_FOUND);
			}
			Map<String, Object> views = (Map<String, Object>) rev
					.getProperties().get("views");
			Map<String, Object> viewProps = (Map<String, Object>) views
					.get(viewName);
			if (viewProps == null) {
				return new TDStatus(TDStatus.NOT_FOUND);
			}
			// If there is a CouchDB view, see if it can be compiled from
			// source:
		...
		if (keys == null) {
			long eTag = options.isIncludeDocs() ? db.getLastSequence()
					: lastSequenceIndexed;
	...
	public TDStatus do_GET_DesignDocument(TDDatabase _db, String designDocID,
			String viewName) {
		return queryDesignDoc(designDocID, viewName, null);
	}

	public TDStatus do_POST_DesignDocument(TDDatabase _db, String designDocID,
			String viewName) {
		Map<String, Object> bodyDict = getBodyAsDictionary();
		if (bodyDict == null) {
			return new TDStatus(TDStatus.BAD_REQUEST);
